        ...
        <div class="flex items-center">
            @if ($report = $class->getReportUrl())
                @if ($class->model?->settings?->raw_width == '25' || $class->model?->settings?->width == 'w-full lg:w-1/4 lg:px-6')
                    <x-link href="{{ $report }}" class="lg:flex hidden text-black hover:bg-gray-100 rounded-xl w-8 h-8 items-center justify-center text-sm text-right" override="class">
                    ...
                    <x-link href="{{ $report }}" class="lg:hidden text-black text-sm text-right" override="class">
                        {{ trans('widgets.view_report') }}
                    </x-link>
                @else
                    <x-link href="{{ $report }}" class="text-black text-sm mr-3 text-right" override="class">
